...
class ConsciousAgent(nn.Module):
    def __init__(self, state_dim: int = Config.Agent.STATE_DIM, hidden_dim: int = Config.Agent.HIDDEN_DIM, vocab_size: int = Config.NLP.VOCAB_SIZE):
        ...
        if self.state_dim > 0:
            ...
            try: self.attention=nn.MultiheadAttention(embed_dim=self.state_dim, num_heads=num_heads, batch_first=True, dropout=0.15); logger.info(f"Agent Attention init {num_heads} heads dim {self.state_dim}.")
        ...
        # --- NEW: Head Movement Prediction Head ---
        ...
        # --- Add head_movement_head parameters to optimizer ---
        agent_params = list(self.korporator.parameters()) + list(self.kaskade.parameters()) + \
                       ...
                       list(self.value_head.parameters()) + list(self.head_movement_head.parameters()) # Add new head params
        ...
        # State history stores combined state
        ...
        self.beta: float = Config.RL.PER_BETA_START; beta_frames: int = Config.RL.PER_BETA_FRAMES; self.beta_increment: float = (1.0 - self.beta) / beta_frames if beta_frames > 0 else 0.0

        # --- Use validated TRAIN_DATA from config.py ---
        if TRAIN_DATA and tokenizer is not None:
            ...
            except Exception as e: logger.error(f"Error during initial GPT training in Agent: {e}", exc_info=True)
        elif tokenizer is None: logger.warning("Skipping initial GPT training: Tokenizer not initialized.")
        else: logger.info("Skipping initial GPT training: No valid training data loaded or found.")
        # --- End GPT Training ---
    ...
    def state_history(self) -> torch.Tensor:
        """Provides the combined state history."""
        if not self.state_history_deque: return torch.zeros(Config.Agent.HISTORY_SIZE, self.state_dim, device=DEVICE, dtype=torch.float32)
        valid_elements = True;
        for t in self.state_history_deque:
             if not isinstance(t, torch.Tensor) or t.shape != (self.state_dim,): logger.error(f"Agent history invalid shape {t.shape}, expected ({self.state_dim},)."); valid_elements = False; break
    ...
    def compute_accessibility(self, history_tensor: torch.Tensor) -> torch.Tensor:
        """Computes accessibility based on combined state history."""
        default_matrix = torch.zeros((Config.Agent.HISTORY_SIZE, Config.Agent.HISTORY_SIZE), device=DEVICE)
        if not isinstance(history_tensor, torch.Tensor) or history_tensor.shape != (Config.Agent.HISTORY_SIZE, self.state_dim): logger.warning(f"Accessibility input shape mismatch: Got {history_tensor.shape}, Expected ({Config.Agent.HISTORY_SIZE}, {self.state_dim})"); return default_matrix
        if not is_safe(history_tensor): logger.warning("Accessibility skipped: Unsafe history."); return default_matrix
        try:
            history_float = history_tensor.float(); norms = torch.linalg.norm(history_float, dim=1, keepdim=True) + 1e-8; normalized_history = history_float / norms; similarity_matrix = torch.matmul(normalized_history, normalized_history.t())
            similarity_matrix = (similarity_matrix + 1.0) / 2.0; similarity_matrix = torch.clamp(similarity_matrix, 0.0, 1.0); accessibility_matrix = torch.where(similarity_matrix > Config.Agent.ACCESSIBILITY_THRESHOLD, similarity_matrix, torch.zeros_like(similarity_matrix))
    ...
    # --- MODIFIED: Return tuple includes head_movement_logits ---
    ...
    def forward(self, state_input: torch.Tensor, current_reward_input: Union[float, torch.Tensor], full_state_history_input: Optional[torch.Tensor]) -> ForwardReturnType:
        """Processes combined state, outputs belief, value, emotions, metrics, and head movement logits."""
        is_batch = state_input.ndim == 2; batch_size = state_input.shape[0] if is_batch else 1
        # Validate input state dimension
        if not isinstance(state_input, torch.Tensor) or state_input.shape[-1] != self.state_dim:
             logger.error(f"Agent.forward: Invalid state type/shape. Expected (*, {self.state_dim}), got {state_input.shape}."); return self._get_default_outputs(batch_size)
        ...
        # Attention mechanism (operates on combined state history)
        history_to_use = None; attn_output_context = discretized_state.clone(); attn_weights = None; att_score = 0.0
        if not is_batch:
            history_to_use = self.state_history # Get combined history
            # Check external history shape
            ...
            if self.attention and history_to_use is not None and history_to_use.shape[0] == Config.Agent.HISTORY_SIZE:
                 ...
                 try:
                     # Attention operates on the full state_dim
                     ...
                     # Validate context shape again
                     if attn_output_context.shape != (self.state_dim,): logger.warning(f"Attn context shape mismatch ({attn_output_context.shape})."); attn_output_context = discretized_state[-1] if discretized_state.ndim == 2 else discretized_state
            ...
            elif not is_batch and self.attention and history_to_use is not None: logger.warning(f"Attn skipped: History size mismatch ({history_to_use.shape[0]})")
        elif is_batch and self.attention:
             attn_output_context = discretized_state # Placeholder for batch attention
             pass

        # Emotional Module (uses only EMOTION_DIM part of state)
        ...
        # Korporator & Kaskade (process combined state/context)
        psi_input = attn_output_context # Use attention context or discretized state
        belief_raw = self.korporator(discretized_state, psi_input, level=2); belief = self.kaskade(belief_raw);
        if not is_safe(belief): belief = torch.zeros_like(belief_raw) # Use shape of raw belief for default

        # Value Head
        value = self.value_head(belief);
        if not is_safe(value): value = torch.zeros_like(value)

        # Feedback Signal (matches combined state dim)
        feedback_signal = self.feedback(belief);
        if not is_safe(feedback_signal): feedback_signal = torch.zeros_like(feedback_signal)

        # --- NEW: Calculate Head Movement Logits ---
        # Use the output of Kaskade (belief) as input
        head_movement_logits = self.head_movement_head(belief)
        if not is_safe(head_movement_logits):
             logger.warning("Unsafe head movement logits. Using zeros.");
             # Ensure default has correct shape
        ...
        # Metrics calculation (remains the same logic, uses combined history if not batch)
        ...
        if is_batch:
            ...
            I_S_norm, rho_struct_val, box_score, R_acc_mean, tau_t = 0.0, 0.0, 0.0, 0.0, Config.Agent.ACCESSIBILITY_THRESHOLD; # No single-step metrics in batch
        else:
            self_consistency = self_consistency_batch.item(); rho_score = rho_score_batch.item()
            if history_to_use is not None: # Calculate metrics based on combined history
    ...
    def _get_default_outputs(self, batch_size=1) -> ForwardReturnType:
         ...
         zero_metrics = (0.0,) * 8
         # Default logits for head movements
    ...
    # --- MODIFIED: Return predicted head movement label string ---
    StepReturnType = Tuple[torch.Tensor, str, torch.Tensor, float, str] # Added string for head movement label

    def step(self, state: torch.Tensor, reward: float, state_history: torch.Tensor, context: Optional[str]) -> StepReturnType:
        """ Processes simulation step, predicts head movement, returns outputs for simulation/memory."""
        self.eval()
        predicted_hm_label = "idle" # Default head movement
        with torch.no_grad():
             # Validate combined state input
             if not isinstance(state, torch.Tensor) or not is_safe(state) or state.shape != (self.state_dim,):
                 logger.warning(f"Agent step invalid state {state.shape}. Exp ({self.state_dim},). Zeros."); state = torch.zeros(self.state_dim, device=DEVICE)
             if state.device != DEVICE: state = state.to(DEVICE)

             # Forward pass returns head movement logits as the last element
             ...
             head_movement_logits = forward_outputs[-1] # Get the logits

             # --- Predict Head Movement Label from Logits ---
             try:
                 # Ensure logits have correct shape before argmax (handle batch dim if it were present)
                 if head_movement_logits.ndim == 1 and head_movement_logits.shape[0] == NUM_HEAD_MOVEMENTS:
                      ...
                      predicted_hm_label = IDX_TO_HEAD_MOVEMENT.get(predicted_hm_idx, "idle") # Map index to label string
                 elif head_movement_logits.ndim == 2 and head_movement_logits.shape[0] == 1 and head_movement_logits.shape[1] == NUM_HEAD_MOVEMENTS: # Handle potential batch dim 1
                      ...
                      logger.warning(f"Unexpected head_movement_logits shape: {head_movement_logits.shape}. Defaulting to 'idle'.")
                      predicted_hm_label = "idle"
             except Exception as e:
                 logger.error(f"Error predicting head movement from logits: {e}")
                 predicted_hm_label = "idle"
             # ---
    ...
    def learn(self, batch_size: int = Config.RL.AGENT_BATCH_SIZE) -> float:
        ...
        # --- Placeholder: Get target head movements IF training ---
        # For now, targets are not retrieved or used, as HEAD_MOVEMENT_LOSS_WEIGHT is 0.0
        # If training, you would need to ensure 'head_movement_idx' is stored in Experience
        # and retrieved here, e.g.:
        # target_hm_indices = batch_data.get('head_movement_idx') # Assuming this key exists if training
        target_hm_indices = None # Keep as None since weight is 0.0
        # ---

        # Validate batch dimensions
        if states.shape[0] != rewards.shape[0] or states.shape[0] != next_states.shape[0] or states.shape[1] != self.state_dim:
             logger.error(f"Learn: Batch dim/state mismatch. States: {states.shape}, Rewards: {rewards.shape}, NextStates: {next_states.shape}, Exp State Dim: {self.state_dim}"); return -1.0
        ...
        try:
            ...
            outputs = self.forward(states, zero_rewards_batch, None) # Pass combined states
            current_value_pred = outputs[3]; rho_score_batch = outputs[8]; box_score_batch = outputs[9]
            head_movement_logits = outputs[-1] # Get HM logits
            ...
            # ... (rho and box validation remains the same) ...
            ...
            if not is_safe(head_movement_logits) or head_movement_logits.shape != (current_batch_size, NUM_HEAD_MOVEMENTS):
                raise ValueError(f"Invalid head_movement_logits shape/safety. Got {head_movement_logits.shape}, Expected ({current_batch_size}, {NUM_HEAD_MOVEMENTS})")
        ...
        with torch.no_grad():
            try:
                ...
                next_outputs = self.forward(next_states, zero_rewards_next_batch, None) # Pass combined next_states
                v_sp = next_outputs[3];
                ...
                # Apply done mask correctly
                dones_squeezed = dones.squeeze(-1) if dones.ndim > 1 else dones
                next_value_pred[dones_squeezed] = 0.0
            except Exception as e: logger.error(f"Error during V(s') calculation in learn: {e}", exc_info=True); next_value_pred = torch.zeros_like(rewards) # Ensure reset on error
        ...
        td_error = target_value - current_value_pred # This is used for PER update

        # --- Calculate Value Loss ---
        value_loss = torch.tensor(0.0, device=DEVICE)
        if current_value_pred.requires_grad:
            # Use Huber loss for value function stability
            value_loss_elementwise = F.smooth_l1_loss(current_value_pred, target_value.detach(), reduction='none', beta=1.0) # Huber loss
            consistency_penalty = (1.0 + (1.0 - rho_score_tensor.clamp(0,1))) # Penalty for inconsistency
            # Apply PER weights and consistency penalty
        ...
        # --- Calculate Head Movement Loss (Placeholder for potential future training) ---
        movement_loss = torch.tensor(0.0, device=DEVICE)
        hm_loss_weight = Config.RL.HEAD_MOVEMENT_LOSS_WEIGHT
        # Only calculate if weight > 0 and targets exist
        if hm_loss_weight > 0 and target_hm_indices is not None and head_movement_logits.requires_grad:
            try:
                # Ensure targets are on the correct device and have the right shape/type
                target_hm_indices_dev = target_hm_indices.to(DEVICE).long()
                if target_hm_indices_dev.shape[0] == current_batch_size:
                    if target_hm_indices_dev.ndim > 1: target_hm_indices_dev = target_hm_indices_dev.squeeze(-1) # Ensure 1D target indices

                    hm_loss_func = nn.CrossEntropyLoss(reduction='none') # Use 'none' for elementwise weighting
                    hm_loss_elementwise = hm_loss_func(head_movement_logits, target_hm_indices_dev)
                    # Apply PER weights to the movement loss as well
                    movement_loss = (hm_loss_elementwise * weights.squeeze(-1)).mean()
                else:
                    logger.warning(f"Learn: Mismatch between batch size ({current_batch_size}) and target head movements ({target_hm_indices.shape}). Skipping HM loss.")
            except Exception as hm_err:
                logger.error(f"Learn: Error calculating head movement loss: {hm_err}", exc_info=True)
                movement_loss = torch.tensor(0.0, device=DEVICE) # Reset loss on error
        # ---
        ...
        # --- Optimization Step ---
        loss_val = total_loss.item() # Use total loss value for reporting
        if is_safe(total_loss) and total_loss.requires_grad and loss_val > 1e-9: # Avoid backward pass for near-zero loss
             self.optimizer.zero_grad()
             total_loss.backward() # Backprop combined loss
             ...
             if agent_params_with_grad:
                 torch.nn.utils.clip_grad_norm_(agent_params_with_grad, max_norm=Config.RL.GRADIENT_CLIP_AGENT)
             else: logger.warning("Learn: No gradients found after backward pass.")
             self.optimizer.step();
             # Update priorities based on TD error (not total loss)
             self.memory.update_priorities(indices, td_error.detach())
        elif not total_loss.requires_grad and abs(loss_val) > 1e-7 :
             logger.debug(f"Learn: Total Loss ({loss_val:.4f}) requires no grad (potentially intended).")
        elif not is_safe(total_loss):
             logger.warning(f"Learn: Unsafe total loss ({loss_val:.4f}). Skipping optimizer step."); self.optimizer.zero_grad()
        else:
             # Loss is zero or safe but no grad required (no update needed)
             pass

        # --- Restore Original Learning Rate if Adaptive was used ---
        if Config.RL.ADAPTIVE_LR_ENABLED and original_lrs:
            for i, param_group in enumerate(self.optimizer.param_groups):
                if i < len(original_lrs): param_group['lr'] = original_lrs[i]

        return loss_val # Return combined loss value
