...
# --- REMOVED TRAIN_DATA from import ---
from config import DEVICE, logger, load_train_data, TRAINING_DATA_PATH, tokenizer, tokenize, detokenize, START_TOKEN_ID, END_TOKEN_ID, PAD_TOKEN_ID
# ---
...
class ConsciousAgent(nn.Module):
    def __init__(self, state_dim: int = Config.Agent.STATE_DIM, hidden_dim: int = Config.Agent.HIDDEN_DIM, vocab_size: int = Config.NLP.VOCAB_SIZE):
        ...
        # --- Load and Validate Training Data Here ---
        loaded_train_data = load_train_data(TRAINING_DATA_PATH)
        validated_train_data: List[Dict[str, Any]] = []
        if loaded_train_data:
            logger.info("Validating and adjusting loaded training data in Agent...")
            for i, item in enumerate(loaded_train_data):
                # Perform validation similar to what was in config.py
                if not isinstance(item, dict) or "output" not in item or "emotion_weights" not in item:
                    logger.warning(f"Invalid item format in loaded TRAIN_DATA at index {i}. Skipping.")
                    continue
                expected_emo_len = 4 # For GPT bias layer
                current_weights = item.get("emotion_weights", [])
                if not isinstance(current_weights, list):
                    logger.warning(f"Invalid emotion_weights type ({type(current_weights)}) in loaded data at index {i}. Skipping.")
                    continue # Skip if weights aren't a list
                # Adjust length
                if len(current_weights) != expected_emo_len:
                    item["emotion_weights"] = (current_weights + [0.0] * expected_emo_len)[:expected_emo_len]
                validated_train_data.append(item) # Add validated item
        # --- End Data Loading/Validation ---

        # --- Use validated_train_data for training ---
        if validated_train_data and tokenizer is not None: # Check if data is valid and tokenizer exists
            try:
                logger.info("Starting initial GPT training...");
                self.gpt.train_model(validated_train_data, epochs=Config.NLP.TRAIN_EPOCHS);
    ...
    # ... (rest of agent.py methods remain the same) ...
    @property
    ...
    def forward(self, state_input: torch.Tensor, current_reward_input: Union[float, torch.Tensor], full_state_history_input: Optional[torch.Tensor]) -> ForwardReturnType:
        ...
        if not is_batch:
            ...
            attn_output_context = discretized_state; pass
        if is_batch:
    ...
    def step(self, state: torch.Tensor, reward: float, state_history: torch.Tensor, context: Optional[str]) -> StepReturnType:
        self.eval()
        with torch.no_grad():
             if not isinstance(state, torch.Tensor) or not is_safe(state) or state.shape != (self.state_dim,): logger.warning(f"Agent step received invalid state. Using zeros."); state = torch.zeros(self.state_dim, device=DEVICE)
