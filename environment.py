...
# utils imports only MetronicLattice and is_safe
...
class EmotionalSpace:
    """Simulates the environment triggering events and providing the *base* state."""
    def __init__(self):
        ...
        logger.debug(f"Environment Lattice initialized with dim: {Config.Agent.BASE_STATE_DIM}")
    ...
    def state_history(self) -> torch.Tensor:
        ...
        if not valid_elements:
             ...
             for _ in range(Config.Agent.HISTORY_SIZE): self.state_history_deque.append(torch.zeros(Config.Agent.BASE_STATE_DIM, device=DEVICE, dtype=torch.float32)) # Use BASE dim
             return torch.zeros(Config.Agent.HISTORY_SIZE, Config.Agent.BASE_STATE_DIM, device=DEVICE, dtype=torch.float32) # Use BASE dim
        try:
    ...
    def step(self) -> StepReturnType:
        ...
        last_valid_base_state = self.state_history_deque[-1].clone() if self.state_history_deque else torch.zeros(Config.Agent.BASE_STATE_DIM, device=DEVICE)

        if base_state is None or not base_state.shape == (Config.Agent.BASE_STATE_DIM,): # Check BASE dim
    ...
    def _get_state(self, current_emotions: torch.Tensor) -> Optional[torch.Tensor]:
        ...
        # Discretize the BASE state using the environment's lattice
        discretized_base_state = self.lattice.discretize(base_state_combined) # Lattice uses BASE_STATE_DIM now
        if not is_safe(discretized_base_state) or discretized_base_state.shape != (Config.Agent.BASE_STATE_DIM,): # Check BASE dim
            logger.error(f"_get_state: Final discretized base state unsafe/wrong shape {discretized_base_state.shape}. Expected ({Config.Agent.BASE_STATE_DIM},)."); return None
    ...
    def get_emotional_impact_from_text(self, text: str) -> torch.Tensor:
        ...
        # Ensure mood vector matches EMOTION_DIM
        if impact_vector.shape[0] != Config.Agent.EMOTION_DIM:
            impact_vector = torch.ones(Config.Agent.EMOTION_DIM, device=DEVICE) * 0.3 # Default base impact

        for emo_idx, (keywords, strength) in self.emotion_keywords.items():
            if emo_idx >= Config.Agent.EMOTION_DIM: continue
            for keyword in keywords:
                if keyword in text_lower:
                    impact_vector[emo_idx] = strength; triggered_strengths.append(strength); found_emotion = True; break
        if not found_emotion and len(impact_vector) == 6: # Only use this default if dimension is 6
