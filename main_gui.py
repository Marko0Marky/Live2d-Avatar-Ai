...
try:
    ...
    from PyQt5.QtCore import Qt, QTimer, pyqtSignal, QEvent # Added QEvent for keyPressEvent type hint
except ImportError as e: logger.critical(f"main_gui.py: PyQt5 import failed: {e}."); raise

from orchestrator import EnhancedConsciousAgent # Already importing the correct orchestrator
...
class EnhancedGameGUI(QMainWindow):
    def __init__(self, agent_orchestrator: EnhancedConsciousAgent):
        ...
        self.last_reward: float = 0.0; self.last_loss: float = 0.0; self.paused: bool = True # Added type hints
        # --- Use nested Config ---
        try:
            self.intensities: List[float] = [float(i) for i in self.agent.env.base_intensities.cpu().tolist()] # Type hint list
            self.event_freq: float = float(Config.Env.EVENT_FREQ) # Access via Config.Env
        except Exception as e:
            logger.error(f"Failed get init env params: {e}")
            # --- Use nested Config in fallback ---
            self.intensities=[0.5]*Config.Agent.EMOTION_DIM; # Access via Config.Agent
            self.event_freq=0.3 # Keep fallback value
        self.completeness_check_interval: int = 200; self.steps_since_last_check: int = 0 # Added type hints
    ...
    def _handle_avatar_init(self, success: bool): # Added type hint
        if success:
            ...
            if not self.timer.isActive():
                # --- Use nested Config ---
                interval=max(10, int(1000.0/Config.Graphics.FPS)) # Access via Config.Graphics
            ...
            # --- Use nested Config ---
            QMessageBox.critical(self, "Init Error", f"Failed Live2D init.\nCheck path '{Config.Graphics.MODEL_PATH}', Core lib, log '{log_file}'."); # Access via Config.Graphics
            self.paused=True; self.timer.stop()

    def _handle_avatar_error(self, error_message: str): # Added type hint
    ...
    def _create_control_group(self): # Added return type hint -> QGroupBox
        ...
        # --- Use nested Config ---
        except Exception: emotion_names=[f"Emo{i+1}" for i in range(Config.Agent.EMOTION_DIM)] # Access via Config.Agent
    ...
    def _create_chat_group(self): # Added return type hint -> QGroupBox
        ...
        layout = QVBoxLayout(); layout.setContentsMargins(0,0,0,0); layout.setSpacing(5) # Use consistent spacing
    ...
    def _update_event_freq(self, value: int): # Added type hint
    ...
    def _update_intensity(self, index: int, value: int): # Added type hint
    ...
    def send_message(self):
        ...
        try:
            ...
            # Ensure emotions passed are valid tensors
            ...
            if not isinstance(response_emotions, torch.Tensor) or not is_safe(response_emotions):
                 ...
                 response_emotions = torch.zeros(Config.Agent.EMOTION_DIM, device=DEVICE) # Fallback
    ...
    def update_game(self):
        ...
        try:
            ...
            if not isinstance(chat_emotions, torch.Tensor) or not is_safe(chat_emotions):
                ...
                # --- Use nested Config ---
                chat_emotions = torch.zeros(Config.Agent.EMOTION_DIM, device=DEVICE) # Fallback
    ...
    def _pause_simulation(self, force_pause: bool = False): # Added type hint
        ...
        if force_pause:
            ...
            elif self.paused: self.state_widget.set_status("Paused (Forced)", "#E67E22") # Update status even if already paused
            return
        if self.paused:
            if can_run:
                self.paused=False;
    ...
    def keyPressEvent(self, event: QEvent): # Added type hint
        key = event.key()
        if hasattr(self, 'chat_input') and self.chat_input.hasFocus():
            if key in (Qt.Key.Key_Return, Qt.Key.Key_Enter):
                self.send_message()
            return # Ignore other keys if chat has focus
        else:
    ...
    def closeEvent(self, event: QEvent): # Added type hint
        ...
        if self.agent and hasattr(self.agent, 'cleanup'):
             ...
             # Fallback avatar cleanup only if orchestrator cleanup fails/missing and avatar exists
