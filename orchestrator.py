...
try:
    ...
    # No print here, logger might not be ready, handled in main.py
# ---

# Use MasterConfig
from config import MasterConfig as Config
from config import DEVICE, logger # Import logger here
# Import Head Movement Labels (needed if storing targets)
from config import HEAD_MOVEMENT_LABELS, HEAD_MOVEMENT_TO_IDX, IDX_TO_HEAD_MOVEMENT # Added IDX_TO_HEAD_MOVEMENT
from environment import EmotionalSpace
from agent import ConsciousAgent
from graphics import Live2DCharacter
# Note: Experience tuple in utils currently doesn't have head_movement_idx
...
# --- MODIFIED: Return tuple includes predicted head movement label ---
TrainStepReturnType = Tuple[ReflectReturnType, float, bool, str, float, str, str] # Added predicted_hm_label string
...
class EnhancedConsciousAgent:
    def __init__(self, train_interval: int = Config.RL.AGENT_TRAIN_INTERVAL, batch_size: int = Config.RL.AGENT_BATCH_SIZE, num_workers: int = 1):
        ...
        if Config.Agent.USE_LANGUAGE_EMBEDDING:
            if SENTENCE_TRANSFORMERS_AVAILABLE:
                try:
                    model_name = Config.NLP.SENTENCE_TRANSFORMER_MODEL; logger.info(f"Loading sentence transformer: {model_name}..."); self.st_model = SentenceTransformer(model_name, device=str(DEVICE))
                    # Dimension Check
                    with torch.no_grad(): # Added no_grad for check
                        test_emb = self.st_model.encode(["test"], convert_to_tensor=True)[0];
                    actual_dim = test_emb.shape[0]; expected_dim = Config.Agent.LANGUAGE_EMBEDDING_DIM
                    if actual_dim != expected_dim: logger.critical(f"FATAL: ST model dim ({actual_dim}) != Config dim ({expected_dim})."); sys.exit(1)
                    logger.info(f"Sentence transformer loaded to {self.st_model.device}. Output dim: {actual_dim}.")
                except Exception as e: logger.error(f"Failed load ST model '{Config.NLP.SENTENCE_TRANSFORMER_MODEL}': {e}. Disabling.", exc_info=True); self.st_model = None; Config.Agent.USE_LANGUAGE_EMBEDDING = False; Config.Agent.__post_init__(Config.Agent); logger.warning(f"Lang embedding disabled. Agent STATE_DIM recalced: {Config.Agent.STATE_DIM}")
            else: logger.warning("SentenceTransformers not found. Disabling language embedding."); Config.Agent.USE_LANGUAGE_EMBEDDING = False; Config.Agent.__post_init__(Config.Agent); logger.warning(f"Lang embedding disabled. Agent STATE_DIM recalced: {Config.Agent.STATE_DIM}")
        else: logger.info("Language embedding in state is disabled via config.")
        ...
        # Ensure mood initialization respects EMOTION_DIM
    ...
    def _get_combined_state(self, base_state: torch.Tensor, context: str = "step") -> torch.Tensor:
        ...
        if base_state_dev.shape[0] != Config.Agent.BASE_STATE_DIM:
             logger.error(f"[{context}] Base state dim mismatch! Exp {Config.Agent.BASE_STATE_DIM}, got {base_state_dev.shape}. Padding/Truncating.");
             padded_base = torch.zeros(Config.Agent.BASE_STATE_DIM, device=DEVICE, dtype=base_state_dev.dtype);
             copy_len = min(base_state_dev.shape[0], Config.Agent.BASE_STATE_DIM); padded_base[:copy_len] = base_state_dev[:copy_len]; base_state_dev = padded_base # Corrected assignment
        if Config.Agent.USE_LANGUAGE_EMBEDDING:
            ...
            return combined
             ...
             # When embedding is disabled, STATE_DIM should equal BASE_STATE_DIM
             if base_state_dev.shape[0] != Config.Agent.STATE_DIM:
                 logger.error(f"[{context}] Lang embed disabled, base dim {base_state_dev.shape[0]} != config state dim {Config.Agent.STATE_DIM}. Critical config error!");
                 # Return zeros of the EXPECTED state dim
    ...
    def _run_learn_task(self) -> Optional[float]:
        ...
        return loss # Return loss or None if execution failed before assignment
        ...
        if self.learn_future and self.learn_future.done():
            try:
                ...
                if loss_result is not None and loss_result >= 0:
                 ...
                 self.last_reported_loss = -1.0 # Indicate error on cancellation
            except Exception as e:
    ...
    # --- MODIFIED: Returns predicted_hm_label ---
    def train_step(self) -> TrainStepReturnType:
        """ Performs internal step, predicts head movement, stores experience, triggers learning, updates mood."""
        ...
        if self.current_state is None or not is_safe(self.current_state) or self.current_state.shape[0] != Config.Agent.STATE_DIM:
            ...
            try:
                ...
                logger.info("State reset successful after invalid state detected.")
                 ...
                 # Return error structure matching the 7-item tuple
                 default_metrics = {'error': True, 'message':"FATAL STATE RESET", "last_monologue":"", "embedding_norm": 0.0, "base_state_norm": 0.0}
                 return (default_metrics, -1.0, True, "FATAL STATE", self.last_reported_loss, "", "idle") # Added "" and "idle" default
        ...
        try: next_base_state, reward, env_done, context_internal, event_type = self.env.step()
             ...
             logger.error(f"Error env step: {e}", exc_info=True);
             next_base_state = base_state_before_step; reward = -0.1; env_done = False; context_internal = "Error env step."; event_type = self.current_event
        if not isinstance(next_base_state, torch.Tensor) or next_base_state.shape[0] != Config.Agent.BASE_STATE_DIM or not is_safe(next_base_state):
            logger.error(f"Env invalid next_base_state (shape {getattr(next_base_state, 'shape', 'None')}). Use prev.");
            next_base_state = base_state_before_step; reward = -0.1; env_done = False; context_internal = "Env State Error"; event_type = self.current_event
        self.last_reward = reward; self.current_event = event_type; self.current_episode_reward_sum += reward

        # --- Agent Step (includes head movement prediction) ---
        att_score_metric = 0.0; response_internal = "(Agent step error)"; emotions_internal = torch.zeros(Config.Agent.EMOTION_DIM, device=DEVICE); predicted_hm_label = "idle" # Default
        belief_for_memory = None # Initialize belief
        try:
            # Agent step now returns predicted head movement label
            ...
            logger.error(f"Error agent step: {e}", exc_info=True);
            ...
            belief_for_memory = torch.zeros(Config.Agent.HIDDEN_DIM, device=DEVICE); # Default belief
            att_score_metric = 0.0; predicted_hm_label = "idle" # Fallback label

        # --- Generate Internal Monologue & Embedding (for next state) ---
        ...
        if Config.Agent.USE_LANGUAGE_EMBEDDING and self.st_model:
             try:
                 ...
                 self.last_internal_monologue = self.model.gpt.generate(monologue_context, emotions_internal, temperature=temp, top_p=top_p, max_len=16) # Adjusted max_len
                 if self.last_internal_monologue and self.last_internal_monologue != "...":
                     # logger.debug(f"Internal Monologue: '{self.last_internal_monologue}'") # Can be verbose
                     with torch.no_grad():
                 ...
                 # Update embedding for NEXT step's combined state construction
        ...
        else: self.last_text_embedding.zero_() # Ensure zero if disabled
        ...
        # Estimate TD error for PER priority
        if is_safe(state_before_step_combined) and is_safe(next_combined_state):
            try:
                ...
                with torch.no_grad():
                     ...
                     if len(outputs_s) == 13 and len(outputs_sp) == 13: # Check for 13 items
                         ...
                         if is_safe(current_value) and is_safe(next_value):
                              ...
                              initial_td_error = abs((target_val - current_value).item()) # Calculate raw TD error
        ...
        if should_store_long_term:
            try:
                ...
                # Calculate final priority using TD error and attention
                ...
                exp = Experience(state=state_before_step_combined.detach(),
                                 ...
                                 reward=reward,
                                 next_state=next_combined_state.detach(), # Store combined next state
                                 done=env_done,
                                 td_error=final_priority) # Store calculated priority
                self.model.memory.add(exp)
            except Exception as e: logger.error(f"Failed adding experience: {e}", exc_info=True)
        # else: logger.debug(f"Memory Gate: Skipped LTM store (Att: {att_score_metric:.3f})")
        ...
        if not self.learn_step_running and (self.total_steps % self.train_interval == 0) and (len(self.model.memory) >= self.batch_size):
            try:
                 # logger.debug(f"Submitting learn task step {self.total_steps}. Mem size: {len(self.model.memory)}")
        ...
        self.current_state = self._get_combined_state(self.current_base_state, "state_update") # Uses embedding from this step
        ...
        if done:
            ...
            try:
                ...
                self.model.prev_emotions.zero_(); self.last_response_emotions.zero_(); self.mood.fill_(0.3); self.conversation_history.clear(); self.episode_steps = 0; self.current_episode_reward_sum = 0.0; self.current_event = self.env.current_event_type;
                 ...
                 return (default_metrics, reward, done, "FATAL RESET", self.last_reported_loss, "", "idle") # Return 7 items

        # --- Return Results ---
        metrics_dict = self.reflect()
        # Return: metrics, reward, done, latest chat response, loss, latest monologue, PREDICTED head movement label
    ...
    def handle_user_chat(self, user_text: str) -> str:
        """Processes user chat, generates response, updates state/avatar."""
        ...
        self.last_text_embedding = user_text_embedding.clone().detach() # Update for next step
        ...
        try:
            ...
            if self.avatar: self.avatar.update_emotions(self.last_response_emotions)
                        ...
                        # Context for GPT
            context_for_gpt = ""
            history_turns = list(self.conversation_history) # Get history as list
            # Iterate in reverse to get recent turns first
            for speaker, text in reversed(history_turns):
                turn = f"{speaker}: {text}\n"
                # Check length BEFORE adding the turn to avoid exceeding limit
                if len(context_for_gpt) + len(turn) > 512: # Approx token limit heuristic
                    break # Stop adding turns if context gets too long
                # Prepend the turn to build context chronologically (newest last)
            ...
            # --- Predict Head Movement After Chat ---
            predicted_chat_hm_label = "idle"
            # Ensure current_state is valid before predicting movement
            if self.current_state is not None and is_safe(self.current_state) and self.current_state.shape[0] == Config.Agent.STATE_DIM:
                try:
                    self.model.eval()
                    with torch.no_grad():
                         # Use the current combined state (which now includes user text embedding)
                         # Use 0 reward for this prediction pass
                         ...
                         if len(outputs) == 13:
                              ...
                              else: logger.warning(f"Unexpected hm_logits shape in chat: {hm_logits.shape}"); idx = HEAD_MOVEMENT_TO_IDX["idle"] # Default index
                    ...
                    self.model.train()
                except Exception as e: logger.error(f"Error predicting head movement after chat: {e}")
            else:
                logger.warning("Skipping chat HM prediction due to invalid current_state.")

            # --- Update Avatar with Chat-Specific Head Movement ---
    ...
    def reflect(self) -> ReflectReturnType:
        """Gathers current statistics and internal states."""
        ...
        try:
            ...
            if current_combined_state is not None and is_safe(current_combined_state) and current_combined_state.shape[0] == Config.Agent.STATE_DIM:
                self.model.eval();
                with torch.no_grad(): reflect_outputs = self.model.forward(current_combined_state, 0.0, self.model.state_history) # Use 0 reward for reflection
                if len(reflect_outputs) == 13: # Check for 13 items
                     (emotions_int, _, _, _value, I_S, _, att_score, self_consistency, rho_score, box_score, R_acc_mean, tau_t, _) = reflect_outputs; # Ignore last item (logits)
    ...
    def test_completeness(self) -> Tuple[bool, str]:
        ...
        elif Config.Agent.EMOTION_DIM == 1: test_state[0] = 0.9 # High Emo 0
        ...
        try:
            ...
            if len(test_outputs) == 13: # Check for 13 items
                 (emotions, _, _, _, _, _, att_score, _, rho_score, box_score, R_acc, _, _) = test_outputs # Ignore HM logits
                 joy_check = False; joy_val = -1.0
                 if Config.Agent.EMOTION_DIM >= 1 and is_safe(emotions):
                      joy_val = emotions[0].item()
                      # Check if Joy is highest AND above threshold
                      joy_check = joy_val > 0.5 and joy_val >= emotions.max().item() - 1e-6 # Allow for small float inaccuracies
    ...
    def cleanup(self):
        ...
        logger.info("--- Orchestrator Cleanup Initiated ---"); logger.info("Shutting down learn executor...");
        try:
             # Cancel pending futures if possible, then shutdown
             # Note: cancel_futures might not work reliably depending on thread state
             self.learn_executor.shutdown(wait=True, cancel_futures=True)
             logger.info("Learn executor shut down.")
        except Exception as e: logger.error(f"Error shutting down learn executor: {e}", exc_info=True)
        # Cleanup Avatar
        if self.avatar and hasattr(self.avatar, 'cleanup'):
             ...
             except Exception as e: logger.error(f"Error avatar cleanup: {e}", exc_info=True)
                # Release ST Model
        if self.st_model:
            try:
                logger.info("Releasing ST model...")
                del self.st_model # Delete the reference
                self.st_model = None # Set attribute to None
                if DEVICE.type == 'cuda':
