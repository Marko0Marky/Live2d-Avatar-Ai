...
import random
import logging
from typing import Optional, Dict, Tuple, List, Deque # Added Deque

# --- Use the instantiated MasterConfig object ---
from config import logger, DEVICE, MasterConfig as Config # Import the instance as Config for easier replacement

# Experience Namedtuple definition (TD error added for PER)
Experience = namedtuple('Experience', ['state', 'belief', 'reward', 'next_state', 'done', 'td_error'])

# --- Helper Function (is_safe) ---
def is_safe(tensor: Optional[torch.Tensor | np.ndarray]) -> bool: # Added type hint
    if tensor is None: return False
    if not isinstance(tensor, torch.Tensor):
        try: tensor = torch.tensor(tensor, device='cpu')
        except Exception: return False
    if not torch.is_floating_point(tensor) and not torch.is_complex(tensor) and not hasattr(tensor, 'is_signed') :
        return True # Assuming non-float/complex/int types are safe
    if tensor.numel() == 0: return True
    if torch.is_floating_point(tensor) or torch.is_complex(tensor):
        return torch.isfinite(tensor).all()
    return True # Assume integers are safe


# --- Metronic Lattice (Uses Config for defaults) ---
class MetronicLattice:
    """Discretizes continuous state space based on Syntrometrie concepts."""
    def __init__(self, dim: int = Config.Agent.STATE_DIM, tau: float = Config.Agent.TAU): # Use nested config
    ...
    def discretize(self, x: torch.Tensor) -> torch.Tensor: # Added type hints
    ...
    def S(self, phi_history: torch.Tensor, n1: int, n2: int) -> torch.Tensor: # Added type hints
        ...
        n2_clamped = max(0, min(n2, history_len - 1))
...
# --- MetaCognitive Memory (Uses Config for defaults and PER) ---
class MetaCognitiveMemory:
    ...
    def __init__(self, capacity: int = Config.Agent.MEMORY_SIZE): # Use nested config
        self.capacity = max(10, capacity)
        self.short_term: Deque[Experience] = deque(maxlen=30) # Type hint deque
        self.long_term: Deque[Experience] = deque(maxlen=self.capacity)
        self.priorities: Deque[float] = deque(maxlen=self.capacity) # Stores priorities (floats)
    ...
    def add(self, experience: Experience):
        ...
        priority = abs(experience.td_error) + 1e-5 # Add epsilon for stability
    ...
    def sample(self, batch_size: int, beta: float = Config.RL.PER_BETA_START) -> Optional[Tuple[Dict[str, torch.Tensor], torch.Tensor, torch.Tensor]]:
         ...
         priorities_float = [float(p) for p in self.priorities]
         priorities = np.array(priorities_float, dtype=np.float64)

         if np.sum(priorities) <= 0 :
             probs = np.ones_like(priorities) / len(priorities) if len(priorities) > 0 else None;
             logger.debug("Memory Sample: Zero priorities detected. Using uniform probabilities.")
         else:
             probs = priorities ** Config.RL.PER_ALPHA
             probs /= probs.sum()

         if probs is None or not np.isclose(probs.sum(), 1.0):
             logger.error(f"Memory Sample: Invalid probabilities calculated (Sum={probs.sum() if probs is not None else 'None'}). Check priorities/alpha.");
             probs = np.ones_like(priorities) / len(priorities) if len(priorities) > 0 else None
             if probs is None: return None

         try:
             indices = np.random.choice(len(self.long_term), batch_size, p=probs, replace=False)
         except ValueError as e:
             logger.error(f"Memory Sample: Error during np.random.choice (check probs sum?): {e}");
             return None
         ...
         for idx in indices:
             ...
             try:
                 ...
                 belief_dev = exp.belief.to(DEVICE) if exp.belief is not None else None;
                 ...
                 beliefs_list.append(belief_to_add);
                 rewards_list.append(exp.reward);
                 next_states_list.append(next_state_dev);
                 dones_list.append(exp.done);
                 valid_count += 1
             except Exception as e:
                 logger.error(f"Memory Sample: Error processing experience index {idx}: {e}");
                 skipped_count += 1
    ...
    def update_priorities(self, indices: torch.Tensor, td_errors: torch.Tensor):
        ...
        if not isinstance(indices, torch.Tensor) or not isinstance(td_errors, torch.Tensor):
             logger.warning(f"update_priorities: Invalid input types ({type(indices)}, {type(td_errors)}).")
             return
        if indices.numel() == 0 or td_errors.numel() == 0 or indices.shape[0] != td_errors.shape[0]:
            logger.warning(f"update_priorities: Mismatch/empty tensors (Indices: {indices.shape}, Errors: {td_errors.shape}). Cannot update.")
            return
    ...
    def get_belief_norm(self, memory_deque: Deque[Experience]) -> float: # Added type hint
        ...
        try:
            individual_norms = [torch.linalg.norm(b.float()).item() for b in valid_beliefs];
